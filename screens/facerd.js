...
import { startActivityForResult } from '../assets/activity';

export default function FaceRD({ route, navigation }) {
    const [userxml, setUserxml] = React.useState('');
    const [userimg, setUserimg] = React.useState('');
    const [username, setUsername] = React.useState('');

    useEffect(() => {
        const { userxml, user } = route.params;
        setUserxml(userxml);
        setUserimg('data:image/png;base64,' + user.image);
    ...
    // const onStart = async () => {
    //     startActivityForResult('android.media.action.IMAGE_CAPTURE',{},(result)=>{
    //         console.log(result);
    //     }).catch((error)=>{
    //         console.log(error);
    //     });
    ...
    // const onStart = async () => {
    //     startActivityForResult('in.gov.uidai.rdservice.face.STATELESS_MATCH', {
    //         request: `<statelessMatchRequest requestId = "${getRandomInt()}" signedDocument = "${userxml}" />`
    //     }).then(result => {
    //         console.log(result);
    //     }).catch(error => {
    //         console.log(error);
    ...
    return (
        <View style={styles.container}>
            <ImageBackground source={require('../assets/Aadhaar_Background.png')} style={styles.appbar}>
                <Text style={styles.title}>FaceRD</Text>
            </ImageBackground>
            <View style={{ width: '90%', paddingVertical: 25, alignItems: 'center' }}>
                ...
                <View style={{ marginTop: '35%' }}>
                    <Text style={styles.begin}>{`Click to Begin\nFacial Recognition`}</Text>
